            ...
            <div class="col-md-12 col-12">
                <div class="card">
                    ...
                    <div class="card-content">
                        ...
                        <form class="form-sample" action="/letters-citizen/{{ $c->uuid }}" method="POST">
                            @method('put')
                                @csrf
                                <div class="form-body">
                                    <div class="row">
                                        ...
                                        <div class="col-md-12 form-group">
                                            ...
                                            <select disabled id="follower_id" class="form-control select2" name="follower_id"
                                                style="width: 100%;" required>
                                                @foreach($citizen as $citizens)
                                                <option value="{{ $citizens->followerUser->id }}">{{ $citizens->followerUser->nik }} -
                                                    {{ $citizens->followerUser->name }}</option>
                                                @endforeach
                                            </select>
                                        </div>

                                        @foreach($letterstreetci as $info)
                                        <div class="col-md-6 form-group">
                                            ...
                                            <input type="text" name="purpose_address"
                                                ...
                                                placeholder="Alamat yang dituju" value="{{ $info->purpose_address }}">
                                        </div>
                                        <div class="col-md-6 form-group">
                                            <label>Keperluan</label>
                                            <input type="text" name="need"
                                                class="form-control @error('need') is-invalid @enderror"
                                                placeholder="Keperluan" value="{{ $info->need }}">
                                        </div>
                                        <div class="col-md-6 form-group">
                                            <label>Tanggal Berangkat</label>
                                            <input type="date" name="date_depart"
                                                class="form-control @error('date_depart') is-invalid @enderror"
                                                placeholder="Tanggal Berangkat" value="{{ $info->date_depart }}">
                                        </div>


                                        <div class="col-md-12 form-group">
                                            <label>Tgl Surat</label>
                                            <input type="date" class="form-control @error('letter_date') is-invalid @enderror" name="letter_date" id="date" required value="{{ $info->letter_date}}"> 
                                        </div>
                                        @endforeach
